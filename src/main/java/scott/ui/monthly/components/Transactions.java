    ...
    public Transactions() {
        ...
        monthAndYear().add(new MonthAndYear.Listener() {
            ...
            public void changed(MonthAndYear monthAndYear) {
                load(monthAndYear().getStartOfMonth(), monthAndYear().getStartOfNextMonth());
                updateEditableState();
            }
        });
...
//		vl.setHeight("1000px");
    ...
    public void notifyCategoryChanged(Category category) {
        if (category != null) {
            ...
            System.out.println("marking rows with category " + category.getName());
    ...
    private BigDecimal getFinalBalance() {
        List<Transaction> transactions = beanContainer.getItemIds();
            if (transactions.size() > 0) {
                  ...
                  return monthAndYear().getMonth().getStartingBalance();
                }
    }
